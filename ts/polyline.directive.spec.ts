...
describe('Polyline Directive', () => {
    ...
    describe('[(geoJSON)]', () => {
        ...
        describe('for LineString', () => {
            ...
            const TEST_LINESTRING: LatLngExpression[] = [
                [0, 0], [1, 0], [1, 1]
            ];
            beforeEach((done) => {
                map = new MapComponent({nativeElement: document.createElement('div')});
            ...
            it('should be changed in Angular when changing in Angular', () => {
                ...
                /* istanbul ignore if */
                if (layer.geoJSON.geometry.type !== 'LineString') {
                ...
                /* istanbul ignore if */
                if (values[0][0] !== TEST_VALUE.geometry.coordinates[0][0] ||
                    values[0][1] !== TEST_VALUE.geometry.coordinates[0][1] ||
                    values[1][0] !== TEST_VALUE.geometry.coordinates[1][0] ||
                    values[1][1] !== TEST_VALUE.geometry.coordinates[1][1] ||
                    values[2][0] !== TEST_VALUE.geometry.coordinates[2][0] ||
                    values[2][1] !== TEST_VALUE.geometry.coordinates[2][1]) {
                    throw new Error(`Wrong value setted: ${ TEST_VALUE } != ${ layer.geoJSON }`);
                }
            });
            it('should be changed geoJSON in Angular when changing in latlngs Leaflet', () => {
            ...
            it('should be changed geoJSON in Angular when adding in latlngs Leaflet', () => {
                ...
                /* istanbul ignore if */
                if ((<GeoJSON.Feature<GeoJSON.LineString>>layer.geoJSON).geometry.coordinates[3][0] !== 3 ||
                    layer.geoJSON.geometry.coordinates[3][1] !== 3) {
                    throw new Error(`Wrong value added: ${ [3, 3] } != ${ layer.geoJSON.geometry.coordinates }`);
                }
            });
            it('should fire an event when changing in Angular', (done: MochaDone) => {
                layer.geoJSONChange.subscribe((eventVal: GeoJSON.Feature<GeoJSON.LineString>) => {
                    /* istanbul ignore if */
                    if (eventVal.geometry.type !== 'LineString') {
